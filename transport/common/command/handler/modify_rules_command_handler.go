...
const (
	...
	WeightRouterRuleType = "weightRouter"
)
...
func (m modifyRulesCommandHandler) Handle(request command.Request) *command.Response {
	...
	if strings.EqualFold(FlowRuleType, typ) {
		...
		return command.OfSuccess(result)
	} else if strings.EqualFold(WeightRouterRuleType, typ) {
		weightRouterRulesInf, err := datasource.WeightRouterRuleJsonArrayParser([]byte(data))
		if err != nil {
			logging.Warn("[modifyRulesCommandHandler] unmarshall error", "data", data, "err", err)
			return command.OfFailure(err)
		}
		var weightRouterRules []*weight_router.Rule
		var ok bool
		if weightRouterRules, ok = weightRouterRulesInf.([]*weight_router.Rule); !ok {
			logging.Warn("[modifyWeightRouterRulesCommandHandler] assert to WeightRouterRulesUpdater error", "data", data)
			err = fmt.Errorf("[modifyWeightRouterRulesCommandHandler] assert to WeightRouterRulesUpdater error")
			return command.OfFailure(err)
		}

		err = datasource.WeightRouterRulesUpdater(weightRouterRules)
		if err != nil {
			logging.Warn("[modifyWeightRouterRulesCommandHandler] WeightRouterRulesUpdater error", "data", data, "err", err)
			return command.OfFailure(err)
		}
		var result = "success"
		if !m.writeToDataSource(util.GetWeightRouterSource(), []byte(data)) {
			result = WriteDsFailureMsg
		}
		return command.OfSuccess(result)
	}
