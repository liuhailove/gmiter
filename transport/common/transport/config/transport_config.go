...
const (
	...
	HeartbeatRemoveDefaultPath = "/registry/removeMachine"
)
...
// GetHeartbeatRemoveApiPath
// Get the heartbeat remove api path. If the machine registry path of the dashboard
// is modified, then the API path should also be consistent with the API path of the dashboard.
func GetHeartbeatRemoveApiPath() string {
	var apiPath = config.HeartbeatRemoveApiDefaultPath()
	if apiPath == "" {
		return HeartbeatRemoveDefaultPath
	}
	if !strings.HasPrefix(apiPath, "/") {
		apiPath = "/" + apiPath
	}
