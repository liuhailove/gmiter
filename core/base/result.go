...
type TokenResult struct {
	...
	linkPass     bool             // 链路传递
	grayTag      string           // 灰度标签
	grayMatchRes string           // 灰度匹配的res,因为灰度时资源允许配置规则中的资源为"*"，这会导致和原始的请求并不一致，因此做此记录
...
// GrayMatchRes 获取规则配置的res
func (r *TokenResult) GrayMatchRes() string {
	return r.grayMatchRes
}
...
func NewTokenResultPassWithGrayResource(refResource *ResourceWrapper, linkPass bool, grayTag string, grayMatchRes string, address []string) *TokenResult {
	return NewTokenResultWithResource(ResultStatusPass, refResource, linkPass, grayTag, grayMatchRes, address)
}
...
func NewTokenResultWithResource(status TokenResultStatus, refResource *ResourceWrapper, linkPass bool, grayTag string, grayMatchRes string, address []string, blockErrOpts ...BlockErrorOption) *TokenResult {
	return &TokenResult{
		status:       status,
		grayRes:      refResource,
		linkPass:     linkPass,
