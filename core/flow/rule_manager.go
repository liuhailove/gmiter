...
type TrafficControllerIdMap map[string]*TrafficShapingController

// ResourceClusterMap 资源->clusterMetric Map
type ResourceClusterMap map[string]*ClusterMetric

var (
	tcGenFuncMap  = make(map[trafficControllerGenKey]TrafficControllerGenFunc, 6)
	tcMap         = make(TrafficControllerMap)
	tcIdMap       = make(TrafficControllerIdMap)
	resClusterMap = make(ResourceClusterMap)
...
func onRuleUpdate(rawResRulesMap map[string][]*Rule) (err error) {
	...
	// 按照ResourceId的Map
	mId := make(TrafficControllerIdMap, len(validResRulesMap))

	// 按照Resource的集群MetricMap
	rClusterMap := make(ResourceClusterMap, len(validResRulesMap))
	...
	for res, rulesOfRes := range validResRulesMap {
		...
		if len(newTcsOfRes) > 0 {
			...
			for _, v := range newTcsOfRes {
				mId[v.rule.ID] = v
				// 集群metric初始化
				if v.rule.isClusterMode() {
					var statIntervalNs int64
					if v.rule.StatIntervalInMs == 0 {
						statIntervalNs = 1000 * MillisToNanosOffset
					} else {
						statIntervalNs = int64(v.rule.StatIntervalInMs) * MillisToNanosOffset
					}
					var maxQueueingTimeNs = int64(v.rule.MaxQueueingTimeMs) * MillisToNanosOffset

					if v.rule.RelationStrategy == AssociatedResource {
						var oldClusterMetric = rClusterMap[v.rule.RefResource]
						if oldClusterMetric != nil {
							// maxQueueTime需要取最小的存入
							if oldClusterMetric.maxQueueingTimeNs > maxQueueingTimeNs || oldClusterMetric.statIntervalNs != statIntervalNs {
								rClusterMap[v.rule.RefResource] = NewClusterMetric(config.MetricStatisticSampleCount(), config.MetricStatisticIntervalMs(), maxQueueingTimeNs, statIntervalNs)
							}
						} else {
							rClusterMap[v.rule.RefResource] = NewClusterMetric(config.MetricStatisticSampleCount(), config.MetricStatisticIntervalMs(), maxQueueingTimeNs, statIntervalNs)
						}
					} else {
						var oldClusterMetric = rClusterMap[v.rule.Resource]
						if oldClusterMetric != nil {
							// maxQueueTime需要取最小的存入
							if oldClusterMetric.maxQueueingTimeNs > maxQueueingTimeNs || oldClusterMetric.statIntervalNs != statIntervalNs {
								rClusterMap[v.rule.Resource] = NewClusterMetric(config.MetricStatisticSampleCount(), config.MetricStatisticIntervalMs(), maxQueueingTimeNs, statIntervalNs)
							}
						} else {
							rClusterMap[v.rule.Resource] = NewClusterMetric(config.MetricStatisticSampleCount(), config.MetricStatisticIntervalMs(), maxQueueingTimeNs, statIntervalNs)
						}
...
func onResourceRuleUpdate(res string, rawResRules []*Rule) (err error) {
	...
	if len(newResTcs) == 0 {
		...
		for k, v := range tcIdMap {
			if v.rule.Resource == res {
				delete(tcIdMap, k)
			}
		}
	} else {
...
func generateStatFor(rule *Rule) (*standaloneStatistic, error) {
	...
	if intervalInMs == 0 || intervalInMs == config.MetricStatisticIntervalMs() {
		...
		retStat.writeOnlyMetric = resNode.DefaultWriteMetric()
		return &retStat, nil
	}
...
// getTrafficControllerFor 根据ID获取trafficController
func getTrafficControllerFor(id string) *TrafficShapingController {
	tcMux.RLock()
	defer tcMux.RUnlock()
	return tcIdMap[id]
}

func getClusterMetric(res string) *ClusterMetric {
	tcMux.RLock()
	defer tcMux.RUnlock()
	return resClusterMap[res]
}
...
func IsValidRule(rule *Rule) error {
	...
	// 如果ControlBehavior[流控效果]为排队等待，则等待时间必须大于0，并且不小于[单机阈值]的倒数
	if rule.ControlBehavior == Throttling {
		if rule.MaxQueueingTimeMs <= 0 || rule.Threshold <= 0 || float64(rule.MaxQueueingTimeMs) <= 1000/rule.Threshold {
			return errors.New("invalid MaxQueueingTimeMs for ControlBehavior, MaxQueueingTimeMs must be great than 0 and Threshold must be great than 0 and MaxQueueingTimeMs  must be great than 1/Threshold")
		}
	}
	...
	if rule.ClusterMode {
		if rule.ClusterConfig == nil {
			return errors.New("cluster mode is true, but cluster config is null")
		}
		var clusterConfig = rule.ClusterConfig
		if clusterConfig.GlobalThreshold <= 0 {
			return errors.New("cluster mode is true, but globalThreshold is null or less than 0")
		}
		if ThresholdGlobal == ClusterStrategy(clusterConfig.ClusterStrategy) {
			// 独立TokenServer的场景下，TokenAddress不能为空
			if TokenServerStrategyIndependentTokenServer == TokenServerStrategy(clusterConfig.TokenServerStrategy) && clusterConfig.TokenServerAddress == "" {
				return errors.New("tokenServer strategy is TokenServerStrategyIndependentTokenServer, but tokenServerAddress is null")
			}
			if TokenServerStrategyNodeSelectMaster == TokenServerStrategy(clusterConfig.TokenServerStrategy) {
				if clusterConfig.FallbackToLocalWhenFail && clusterConfig.DowngradeDurationInMs <= 0 {
					return errors.New("tokenServer strategy is TokenServerStrategyNodeSelectMaster, downgradeDurationInMs must be greater than 0")
				}
				//if clusterConfig.MasterNodeThreshold <= 0 {
				//	return errors.New("tokenServer strategy is TokenServerStrategyNodeSelectMaster, masterNodeThreshold must be greater than 0")
				//}
			}
		}
	}
